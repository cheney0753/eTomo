...
if __name__ == '__main__':
    ...
    rec_hd = data_hd.reconstruct( norm = 'L2', regularization = 'TV', alg = 'PDHG',  n_iter = 100, lmbd = 10)
    ...
    scms.imsave(os.path.join(save_dir, '002_haadf_tv.png'), rec_hd/rec_hd.max())

    element = elements[0]
    data_eds = Projection_data( dnp_e[element], geom_e)
    data_eds.normalize()
    rec_eds = data_eds.reconstruct( norm = 'KL', regularization = 'TNV', alg = 'DR', n_iter = 100, lmbd = 0.1, rec_sub = rec_hd)

    scms.imsave(os.path.join(save_dir, '002_{}_tnv.png'.format(element)), rec_eds/rec_eds.max())
