...
#    dim = '3D'
    ...
    # smooth the EDS data
    p_e = smooth_gaussian(p_e, sigma = 2.4)
...
#    data_h.normalize() # Normalize the data before reconstruct
    ...
    # extra-line
    rec_h = data_h.reconstruct( norm = 'L2', regularization = 'TV', alg = 'DR',  n_iter = 200, lmbd = 0.001)
    rec_h += rec_h.max()/100.0 #TODO: find out why
    
    data_e = Projection_data(dnp_e, geom_e)
    data_e.normalize_01() # Normalize the data before reconstruct
#    data_e.normalize() # Normalize the data before reconstruct

    #==================
    # save the figure
    #==================

    rec_e = data_e.reconstruct(norm = 'L2', regularization = 'TNV', alg = 'DR', n_iter = 100, lmbd = 0.001, rec_sub = rec_h)

    scms.imsave(os.path.join(save_dir, '005_{}_tnv.png'.format('Ti')), rec_e/rec_e.max())
    scms.imsave(os.path.join(save_dir, '005_{}_tnv.png'.format('HAADF')), rec_h/rec_h.max())
